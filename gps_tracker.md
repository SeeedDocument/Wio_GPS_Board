...
What is GPS Tracker? GPS Tracker is an Arduino compatible development board that helps you to track any moving things on the planet. By integrating GSM&GPRS as well as GPS&BeiDou in one board, it provides an all-in-one solution for your out-door IoT project.

If you are following the latest news about Low-Power Wide-Area Network(LPWAN), you may know that 2016 is a special year because there are new technology like LoRa and Sigfox came out, which remarkably pushed the development of WAN. The GPS Tracker has similarities as LoRa and Sigfox but also quite different. It is more suitable for out-door moving tracking and situations that has high frequency customer interaction and medium volume of data, for example sharing bicycle, logistics location, pet tracking.

Comparing to traditional GPS tracking solution, Seeed’s GPS Tracker is much more easier to use and customize for the fast changing IOT market. As a user-friendly Grove development board, the look of GPS tracker may remind you of Wio Link, another started product of Seeed. So you can also find 6 grove connector on the board. As an Arduino compatible board, users can use the GPS Tracker with Arduino IDE directly.

Why do we call the GPS tracker a solution, because it is really not just a board. At SeeedStudio, we have the ability to help customer customize the GPS tracker for their own project and finish the project all the way to mass production. As GPS tracker uses standard module that is mass production, we are ready to mass production even when customer is at their prototyping stage.

We have prepared everything, the last thing we need is your creative projects, so if you are making an out-door IOT tracking project, please don’t hesitate to use the GPS Tracker!
...
!!!Warning
    Unlike most Arduino & Genuino boards, the GPS Tracker runs at 3.3V. The maximum voltage that the I/O pins can tolerate is 3.3V. Applying voltages higher than 3.3V to any I/O pin could damage the board.
...
When the first time to insert the board, you should get a USB COM device name GPS Tracker that need to install a driver. Click [here](https://github.com/SeeedDocument/Seeeduino_LoRa/raw/master/res/driver.zip) to download driver for the board.



To make sure the driver was installed successful, open your Device Manager to see if **GPS Tracker** exists.

### Install the library of GPS Tracker(For Windows, Linux and Mac)

Click [here](https://github.com/lanselambor/MC20_GPS_Tracker/archive/master.zip) to download the library(zip file) of GPS Tracker and import the zip library into your Arduino IDE. If you are new to this, please click [here](https://www.arduino.cc/en/Guide/Libraries#toc4) for more information.


### Blink

Different from other boards, there is an User LED SK6812 on GPS Tracker, which is an intelligent control LED light source and similar to ws2812b. Now we are going to upload our first demo - Blink to GPS Tracker, which will introduce how to use the User LED.
...
* Click on **Tools > Board > GPS Tracker**
...
GPS Tracker is designed for tracking moving things outdoor. So you can power the board via a 3.7V Lipo battery with JST1.0 connector and check the voltage of your battery easily.
...
To use 6 Grove connectors on GPS Tracker, we have to use `digitalWrite(12, HIGH)` to open 3V3\_B to power the Grove modules(D12 defaults to LOW in order to reduce consumption).
...
And we have installed a well written library for the GPS Tracker, for simple applications you even don't need to know much about how to use the AT commands, which is complex and hard to read.

The following example shows how to read longitude and latitude and print them out in Serial Monitor. Please note that GPS Tracker should be placed outdoor to get GPS signal.
...
The following example shows how to read sms messages on GPS Tracker. Nano SIM card is needed in this example.
...
It is also very easy to connect to your Bluetooth device on GPS Tracker. Open your Arduino IDE and click on **File > Examples > MC20\_GPS\_Traker > MC20\_BlueTooth > BT\_FastConnect** to open the sketch or copy the blow code.

Then change the deviceName of your Bluetooth device, GPS Tracker will connect it automatically.
